...
| `403`  | Key disabled / IP or origin restrictions              |
...
| 15  | `GET`    | `/api/account`                    | Corridor currency & store profile summary       |
| 16  | `GET`    | `/api/exchange-rate`              | FX rate between two ISO 4217 codes              |
| 17  | `GET`    | `/api/convert`                    | Convert amount between two currencies           |
...
| `CHECKOUT`          | Returns hosted checkout fields when you owe an upfront share. Redirect using `authorization_url` or `checkout_url`. Matching for paid flows starts after payment confirms (webhook). |
...
```json
{
  ...
  "data": {
    ...
    "p2pDelivery": { "id": "p2p-uuid", "status": "PENDING", "type": "PICKUP" }
  }
}
```
    ...
    "data": [
      {
        ...
        "deliveryFeeDisplay": {
          "usd": 4.75,
          "local": { "currency": "NGN", "amount": 7125, "symbol": "₦" }
        },
        "p2pDelivery": {
          "id": "uuid",
          "totalValue": "100.00",
          "totalValueDisplay": { "usd": 100, "local": { "currency": "NGN", "amount": 150000 } }
        },
