...
Errors are returned using the global error envelope described in [Errors](../get-started/errors.mdx).
...
Origin contact fields are optional (`originContactEmail`, `originContactPhone`, `originContactName`) and default to your user’s info.
...
Uses the same request body as the P2P package endpoint (`CreateOrEditPackageDTO`).
